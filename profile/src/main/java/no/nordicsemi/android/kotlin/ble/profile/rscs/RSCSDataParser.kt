...
object RSCSDataParser {
    ...
    fun parse(byteArray: ByteArray): RSCSData? {
        ...
        val speed = data.getIntValue(ByteData.FORMAT_UINT16_LE, offset)?.toFloat()?.let {
            it / 256f // [m/s]
        } ?: return null

        offset += 2
        val cadence: Int = data.getIntValue(ByteData.FORMAT_UINT8, offset) ?: return null
        offset += 1
        ...
        if (instantaneousStrideLengthPresent) {
            strideLength = data.getIntValue(ByteData.FORMAT_UINT16_LE, offset)
            offset += 2
        }

        var totalDistance: Long? = null
        if (totalDistancePresent) {
            totalDistance = data.getLongValue(ByteData.FORMAT_UINT32_LE, offset)
            // offset += 4;
        }
